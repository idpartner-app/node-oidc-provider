...
export default (provider) => class Session extends hasFormat(provider, 'Session', instance(provider).BaseModel) {
  ...
  static async findByUid(uid) {
    ...
    // console.log('gio: stored session:', JSON.stringify(stored));
    if (!stored) {
      // console.log('gio: returning undefined because stored session was not found');
      return undefined;
    }
    try {
      const payload = await this.verify(stored);
      // console.log('gio: payload:', JSON.stringify(payload));

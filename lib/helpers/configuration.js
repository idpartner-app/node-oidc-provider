...
class Configuration {
  ...
  // Collect verified claims and their metadata so it's easier to expose it in the well-known endpoint.
  // Spec taken from https://openid.net/specs/openid-connect-4-identity-assurance-1_0.html#name-op-metadata
  collectVerifiedClaims() {
    this.isVerifiedClaimsSupported = this.verifiedClaims.enabled;

    if (!this.verifiedClaims.enabled) return;

    // TODO:
    //  - some of the attributes marked as required below are optional in the spec. Update them to not be required
    //  - add support for attachments (attachments_supported and digest_algorithms_supported) mentioned in the spec link mentioned above
    const evidenceTypes = ['document', 'electronic_record', 'vouch', 'utility_bill', 'electronic_signature'];
    const verifiedClaimsSupported = new Set();
    const trustFrameworksSupported = new Set();
    const evidenceSupported = new Set();
    const documentTypesSupported = new Set();
    const documentMethodsSupported = new Set();
    const documentChecksSupported = new Set();
    const electronicRecordsSupported = new Set();

    this.verifiedClaims.claims.forEach((claim) => {
      // Collect nested verification attribtues
      const verification = claim.verification;

      if (!verification.trustFramework) throw new Error(`claims[].trustFramework must be defined in ${JSON.stringify(claim)}`);
      trustFrameworksSupported.add(claim.verification.trustFramework);

      if (!verification.evidence || !Array.isArray(verification.evidence)) throw new Error(`claims[].evidence[] must be defined in ${JSON.stringify(claim)}`);
      verification.evidence.forEach(evidence => {
        if (!evidence.type) throw new Error(`claims[].evidence[].type must be defined in ${JSON.stringify(claim)}`);
        if (!evidenceTypes.includes(evidence.type)) throw new Error(`claims[].evidence[].type must be one of ${supportedEvidenceTypes.join(',')} in ${JSON.stringify(claim)}`);
        evidenceSupported.add(evidence.type)

        if (evidence.type === 'document') {
          if (!evidence.method) throw new Error(`claims[].evidence[].method must be defined for evidence of type "document" in ${JSON.stringify(claim)}`);
          documentMethodsSupported.add(evidence.method);

          if (!evidence.documentDetails) throw new Error(`claims[].evidence[].documentDetails must be defined for evidence of type "document" in ${JSON.stringify(claim)}`);
          if (!evidence.documentDetails.type) throw new Error(`claims[].evidence[].documentDetails.type must be defined for evidence of type "document" in ${JSON.stringify(claim)}`);
          documentTypesSupported.add(evidence.document_details.type);

          if (!evidence.checkDetails || !Array.isArray(evidence.checkDetails)) throw new Error(`claims[].evidence[].checkDetails[] must be defined for evidence of type "document" in ${JSON.stringify(claim)}`);
          evidence.checkDetails.forEach(checkDetail => {
            if (!checkDetail.check_method) throw new Error(`claims[].evidence[].checkDetails[].check_method must be defined for evidence of type "document" in ${JSON.stringify(claim)}`);
            documentChecksSupported.add(checkDetail.check_method);
          });
        }

        if (evidence.type === 'electronic_record') {
          if (!evidence.record) throw new Error(`claims[].evidence[].record must be defined for evidence of type "electronic_record" in ${JSON.stringify(claim)}`);
          if (!evidence.record.type) throw new Error(`claims[].evidence[].record.type must be defined for evidence of type "electronic_record" in ${JSON.stringify(claim)}`);
          electronicRecordsSupported.add(evidence.document_details.type);
        }
      })

      // Collect verified claims
      Object.keys(claim.claims).forEach(claimName => verifiedClaimsSupported.add(claimName));
    })

    // Set the collected data to variables that can be easily accessed from the discovery.js file
    ...
    this.documentTypesSupported = documentTypesSupported;
    this.documentMethodsSupported = documentMethodsSupported;
    this.documentChecksSupported = documentChecksSupported;
